...
class BrushConfig:
    ...
    def __init__(self, config: dict, process_site_config=True):
        ...
        self.include_labels = config.get("include_labels")
        self.exclude_labels = config.get("exclude_labels")
...
class BrushFlow(_PluginBase):
                                                ...
                                                'content': [
                                                    {
                                                        ...
                                                        'props': {
                                                            ...
                                                            'label': '包含标签',
                                                            'placeholder': '如：Free,官方（多个用英文逗号分隔）'
                                                        }
                                                    }
                                                ]
                                            },
                                                ...
                                                'content': [
                                                    {
                                                        ...
                                                        'props': {
                                                            ...
                                                            'label': '排除标签',
                                                            'placeholder': '如：无字幕,DIY（多个用英文逗号分隔）'
                                                        }
                                                    }
                                                ]
                                            }
    ...
    def __evaluate_conditions_for_brush(self, torrent, torrent_tasks) -> Tuple[bool, Optional[str]]:
        ...
        # 包含标签规则
        if brush_config.include_labels:
            include_label_list = [label.strip() for label in brush_config.include_labels.split(',') if label.strip()]
        ...
        if brush_config.exclude_labels and torrent.labels:
            exclude_label_list = [label.strip() for label in brush_config.exclude_labels.split(',') if label.strip()]
            if exclude_label_list:
                # 检查种子的labels是否包含任意一个排除的标签
                for label in exclude_label_list:
                    if label in torrent.labels:
                        return False, f"包含排除标签：{label}"
